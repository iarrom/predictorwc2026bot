...
export function BracketStageNavigator({
  ...
  return (
    <div className="shrink-0 px-4 pb-4 pt-2">
      <div className="relative rounded-full bg-[rgb(4_12_48/0.9)] px-1 py-1">
        <div
          className="pointer-events-none absolute inset-y-1 rounded-full border border-white/80 transition-[left,width] duration-300 ease-out motion-reduce:transition-none"
          style={{
        ...
        <div className="relative grid grid-cols-5 items-center gap-1 px-6">
          <button
            type="button"
            onClick={scrollPrev}
            disabled={activeIndex === 0}
            className="absolute top-1/2 left-0 flex size-7 -translate-y-1/2 items-center justify-center text-white/70 disabled:opacity-30"
          ...
          {KNOCKOUT_BRACKET_STAGES.map((stage) => {
            ...
            return (
              <button
                ...
                className={cn(
                  "flex items-center justify-center py-2.5 text-center text-[11px] font-semibold tracking-wide transition-colors",
                  highlighted ? "text-white" : "text-white/35 hover:text-white/55",
                )}
          ...
          <button
            ...
            className="absolute top-1/2 right-0 flex size-7 -translate-y-1/2 items-center justify-center text-white/70 disabled:opacity-30"
