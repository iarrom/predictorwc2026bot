...
export interface LeaderboardOverallEntry extends LeaderboardPlayer {
  ...
  tiebreaker_overall: number | null;
  rank: number;
}
...
export interface LeaderboardStageEntry extends LeaderboardPlayer {
  points: number;
  picks: number;
  tiebreaker_deviation: number | null;
  rank: number;
}
...
export function stageKeyToTiebreakerRound(stageKey: string): TiebreakerRoundKey {
  if (
    stageKey === "group_1" ||
    stageKey === "group_2" ||
    stageKey === "group_3"
...
function getStageTiebreakerDeviation(
  userId: string,
  stageKey: string,
  roundDeviationByUser: Map<string, Record<TiebreakerRoundKey, number | null>>,
): number | null {
  return (
    roundDeviationByUser.get(userId)?.[stageKeyToTiebreakerRound(stageKey)] ??
    null
  );
}

function compareStageRanked(
  a: {
    points: number;
...
export function buildLeaderboardAnalytics(input: {
  ...
  tiebreakerRoundDeviationByUser?: Map<
    string,
  ...
  const {
    ...
    tiebreakerRoundDeviationByUser,
  } = input;
  const tiebreakerByUser = tiebreakerOverallByUser ?? new Map<string, number | null>();
  const roundDeviationByUser =
    tiebreakerRoundDeviationByUser ??
    new Map<string, Record<TiebreakerRoundKey, number | null>>();
  ...
  const overall = assignRanks(
    players
      .map((player) => ({
        ...
        tiebreaker_overall: tiebreakerByUser.get(player.user_id) ?? null,
        rank: 0,
      }))
      .sort((a, b) =>
    ...
    perStage[stageKey] = assignRanks(
      players
        .map((player) => ({
          ...
          tiebreaker_deviation: getStageTiebreakerDeviation(
            player.user_id,
            stageKey,
            roundDeviationByUser,
          ),
          rank: 0,
        }))
        ...
        .sort((a, b) =>
          compareStageRanked(
            {
              points: a.points,
              tiebreaker: a.tiebreaker_deviation,
              display_name: a.display_name,
            },
            {
              points: b.points,
              tiebreaker: b.tiebreaker_deviation,
              display_name: b.display_name,
            },
          ),
        ),
